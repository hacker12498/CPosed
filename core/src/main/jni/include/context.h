...
namespace lspd {
    ...
    public:
        ...
        inline clant::ScopedLocalRef<jclass>
    ...
    protected:
        ...
        static clant::ScopedLocalRef<jclass> FindClassFromLoader(JNIEnv *env, jobject class_loader,
        ...
        inline void FindAndCall(JNIEnv *env, std::string_view method_name, std::string_view method_sig,
            ...
            jmethodID mid = clant::JNI_GetStaticMethodID(env, entry_class_, method_name, method_sig);
            if (mid) [[likely]] {
                env->CallStaticVoidMethod(entry_class_, mid, clant::UnwrapScope(std::forward<Args>(args))...);
            } else {
                LOGE("method {} id is null", method_name);
            }
        }

        virtual void InitArtHooker(JNIEnv *env, const clant::InitInfo &initInfo);
